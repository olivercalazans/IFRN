...
while true; do
    ...
    echo "3. Adicionar IP ao iptables NAT"
    echo "0. Sair"
	...
	"3")
	    while true; do
		clear
		echo 'a. Para adicionar IP'
		echo 'd. Para remover IP'
		read -p '>> ' opcao3
		opcao3=$(echo $opcao3 | tr '[:lower:]' '[:upper:]')

		if [[ $opcao3 == A || $opcao3 == D ]]; then
		    read -p 'Informe o IP: ' ip
		    iptables -t nat -$opcao3 POSTROUTING -s $ip -j MASQUERADE
		    clear
		    echo
		    echo '===================== IPTABLES ====================='
		    iptables -t nat -nL
